...
|--------|---------|-----------|
| `main` | Production-ready code | `https://nexora.dev` |
...
vercel deploy --prebuilt --prod
    │
    ▼
curl smoke test: GET https://nexora.dev → assert HTTP 200
...
| `RESEND_FROM_EMAIL` | Same all tiers | `no-reply@nexora.dev` | Same | |
| `RESEND_NOTIFY_EMAIL` | Your personal email | Your personal email | `hello@nexora.dev` | Preview sends to dev, not team inbox |
| `NEXT_PUBLIC_SITE_URL` | `http://localhost:3000` | Preview URL | `https://nexora.dev` | Used in canonical URLs and sitemap |
...
`lhr1` = London Heathrow. Closest Vercel edge region to the European market. Server-side rendering and serverless functions execute here by default.
